...
namespace ORB_SLAM2
{
    // 前端，跟踪线程，完成特征检测与匹配，位姿初始计算，地图点三角化
    // 参数：主线程指针，字典指针，frame画图类指针，画地图类指针，地图指针，关键帧数据库，配置文件路径，传感器类型
    // 主要工作：
    // 1. 装载相机配置参数
    // 2. 装载ORB的特征提取参数
    // 3. 构建特征提取器，针对不同方案
    ...
    {
        // step 1 装载配置参数
        // step 1.1 相机内参数，畸变参数，基线（双目时），相机的帧率，图像格式（RGB，BGR，or gray）
        ...
        // 单目没有bf参数，那么mbf会是什么值？单目不用mbf，所以是什么都没关系
        // 双目摄像头baseline * fx 50
    ...
    cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
    {
        ...
        // 步骤2：构造Frame，完成orb特征提取
        if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET) // 没有成功初始化的前一个状态就是NO_IMAGES_YET
            // 干事前回答两个问题，1. 初始化了吗？ 2. 有图像数据了吗？
            // 有一个不满足，那就区别对待了
            // 特征就需要取得多一些，多多少呢？多一倍
    ...
    void Tracking::Track()
    {
        ...
        // 地图的线程不能更新，直到Track函数运行完
        // Get Map Mutex -> Map cannot be changed
        unique_lock<mutex> lock(mpMap->mMutexMapUpdate);

        // 步骤1：初始化
        // 初始化完成获得两个关键帧，及它们间的位姿，匹配的特征点，对应的3D点
        if (mState == NOT_INITIALIZED)
        {
            if (mSensor == System::STEREO || mSensor == System::RGBD)
                StereoInitialization();
            else
                MonocularInitialization(); // 单目初始化，成功则更新mState状态

            mpFrameDrawer->Update(this); // 绘图线程更新数据
        ...
        else // 步骤2：跟踪
        {
            ...
            if (!mbOnlyTracking)
            {
                ...
                if (mState == OK)
                {
                    // 新来的帧，用参考关键帧还是上一个普通帧来跟踪它呢？
                    // 1. 如果上一帧没有运动，那么就用参考帧
                    // 2. 如果有运动，假设运动是恒速的，用运动模型来与上一个普通帧跟踪
            ...
            if (!mbOnlyTracking)
            {
                // bOK指上面的操作成功
                if (bOK)
                    // 用关键帧的共视关系，搜索出相邻的局部关键帧，用这些共视的地图点，优化当前帧的位姿
                    bOK = TrackLocalMap();
            }
            else
            {
    ...
    void Tracking::MonocularInitialization()
    {
        ...
        if (!mpInitializer)
        {
            ...
            if (mCurrentFrame.mvKeys.size() > 100)
            {
                ...
                // TODO mInitialFrame 和 mLastFrame 相同啦？咋回事？
                ...
                // 这两句是多余的
                // 确实多余，mpInitializer有值的话，就执行else了
                if (mpInitializer)
        ...
        else
        {
            ...
            {
                // 删掉之后，初始帧mInitialFrame变为当前帧啦，再找下一帧
                // 也就是说，必须用两个相邻的普通帧，做初始化
                delete mpInitializer;
            ...
            vector<bool> vbTriangulated; // Triangulated Correspondences 是否能够进行三角化 (mvIniMatches)
    ...
    // 丢了，当前帧与关键帧库进行匹配，找到一些共视帧，计算位姿
    // 关键帧的地图点不再优化，只优化它们与当前帧的位姿
    bool Tracking::Relocalization()
    {
        // Compute Bag of Words Vector
        // 步骤1：计算当前帧特征点的Bow映射
        // 关键帧里有特征点信息，这里用词袋加速匹配
        mCurrentFrame.ComputeBoW();
        ...
        for (int i = 0; i < nKFs; i++)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // 3D关键帧中的地图点
                    // 2D是当前帧的像素点
                    // 与每个匹配的关键帧算一个位姿，这里只是初始化了求解器，还没算
