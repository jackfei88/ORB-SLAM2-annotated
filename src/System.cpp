...
{
    // SLAM入口类
    // 参数：字典路径，配置文件路径，传感器类型，是否实时显示
    // 做了哪些工作
    // 1. 读入配置文件
    // 2. 读入字典
    // 3. 初始化相关类
    // 4. 初始化线程，4个线程：跟踪线程，局部地图线程，闭环线程，显示线程
    System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor, const bool bUseViewer) : mSensor(sensor), mbReset(false), mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false)
    {
        ...
        // 跟踪是在主线程中的，完成特征检测与匹配，位姿初始估计，三角化
        mpTracker = new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer,
                                 mpMap, mpKeyFrameDatabase, strSettingsFile, mSensor);

        // step 4.2 初始化局部地图线程，后端？
    ...
    cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp)
    {
        ...
        {
            unique_lock<mutex> lock(mMutexMode);
            // mbActivateLocalizationMode和mbDeactivateLocalizationMode开始时是false
            // TODO 什么时候变得true？
            if (mbActivateLocalizationMode)
            {
                mpLocalMapper->RequestStop();
